...
player_img = pygame.transform.scale(player_img, (60, 60))  # 크기 조절
print(resource_path("assets/player.png"))

bullet_img = pygame.image.load(resource_path("assets/bullet.png"))
bullet_img = pygame.transform.scale(bullet_img, (20, 40))  # 크기 조절
...
hit_sound = pygame.mixer.Sound(resource_path("assets/hit.wav"))
...
def draw_player(surf, rect, angle):
    # 🔥 이미지 회전 (마우스 방향)
    rotated = pygame.transform.rotate(player_img, -math.degrees(angle))
    ...
    offset = 25
    fx = rect.centerx + math.cos(angle_corrected) * offset
    fy = rect.centery + math.sin(angle_corrected) * offset

    flame_rect = rotated_flame.get_rect(center=(fx, fy))
    surf.blit(rotated_flame, flame_rect)
...
def draw_enemy(surf, en):
    ...
    # 🔥 회전
    rotated = pygame.transform.rotate(enemy_img, -math.degrees(angle) + 90)
...
def draw_hud(score, lives, remaining):
    screen.blit(font.render(f"Score: {score}", True, WHITE), (10, 10))
    screen.blit(font.render(f"Lives: {'♥ ' * lives}", True, RED), (WIDTH - 180, 10))

    minutes = remaining // 60
    sec = remaining % 60
    time_text = f"{minutes:02}:{sec:02}"

    screen.blit(font.render(time_text, True, YELLOW), (WIDTH // 2 - 40, 10))
...
def win_screen():
    screen.fill((10, 10, 30))
    screen.blit(font_big.render("YOU WIN!", True, GREEN), (250, 220))
    screen.blit(font.render("R: Restart   Q: Quit", True, WHITE), (270, 320))
    pygame.display.flip()
...
def get_bullet_scale(score):
    return min(2.5, 1 + score / 200)  # 최대 2.5배
...
def main():
    ...
    while True:
        ...
        spawn_timer += 1
        # 시간이 지날수록 spawn 간격 감소 (최소값 제한)
        dynamic_spawn = max(10, level_cfg["spawn"] - time_passed // 300)

        if spawn_timer >= dynamic_spawn:
            spawn_timer = 0
        ...
        for bi, b in enumerate(bullets):
            for ei, en in enumerate(enemies):
                scale = get_bullet_scale(score)

                w = int(bullet_img.get_width() * scale)
                h = int(bullet_img.get_height() * scale)

                temp = pygame.Surface((w, h), pygame.SRCALPHA)
                temp.blit(pygame.transform.scale(bullet_img, (w, h)), (0, 0))

                rotated = pygame.transform.rotate(temp, -math.degrees(b["angle"]) + 90)
                bullet_rect = rotated.get_rect(center=(b["x"], b["y"]))
        ...
        for b in bullets:
            ...
            w = int(bullet_img.get_width() * scale)
            h = int(bullet_img.get_height() * scale)
            scaled = pygame.transform.scale(bullet_img, (w, h))

            # 회전
            rotated = pygame.transform.rotate(scaled, -math.degrees(b["angle"]) + 90)
        ...
        if blink:
            draw_player(screen, player, angle + -80)

        elapsed = time_passed // FPS
        remaining = max(0, TARGET_TIME - elapsed)

        draw_hud(score, lives, remaining)
